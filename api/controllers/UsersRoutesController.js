...
module.exports = {
  ...
  create: function (req, res) {
    ...
    const process = async () => {
      const countUsersRoutes = await UsersRoutes.count({
        where: { user: req.param('user_id'), routes: req.param('routes_id'), }
      });
      if (countUsersRoutes >= 1) {
  ...
  find: function (req, res) {
    ...
    const getUsersRoutes = async () => {
      ...
      if (UsersRoutes_count < 1) {
        throw new CustomError('Users Route not found', {
          status: 403
        });
      }
  ...
  findOne: function (req, res) {
    ...
    const getUsersRoutes = async () => {
      ...
      if (usersRoutes)
        ...
        throw new CustomError('UsersRoutes not found', {
          status: 403
        });
  ...
  update: function (req, res) {
    ...
    const updateUsersRoutes = async () => {
      ...
      if (oldUsersRoutes < 1) {
        throw new CustomError('Invalid UsersRoutes  Id', {
          status: 403
        });
      }
      ...
      if (req.param('description') != undefined && _.isString(req.param('description'))) {
        usersRoutes.description = req.param('description');
      }
  ...
  delete: function (req, res) {
    ...
    const deleteUsersRoutes = async () => {
      ...
      if (checkUsersRoutes < 1) {
        throw new CustomError('Invalid Users Routes Id', {
          status: 403
        });
      }
