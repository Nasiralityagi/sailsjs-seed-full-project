...
module.exports = {
  ...
  create: async function (req, res) {
    ...
    // console.log(calculator.calculate(joinArray));
    ...
    const process = async () => {
      ...
      if (newConnRenewal) {
        // console.log(newConnRenewal);
        const newConnection = await Connection.findOne({ id: req.param('connection_id') }).populate('packages');
        ...
        if (newConnection) {
          ...
          if (workflow) {
            const newJournalEntry = await JournalEntry.create({
              ...
              'user_remarks': 'Data Inserted through workflow',
            ...
            for (let w of workflow) {
              ...
              for (let v of variableArr) {
                switch (v) {
                  case 'amount':
                    formulaString = formulaString.replace('amount', newConnRenewal.renewal_price);
                    break;
                  case 'cost_price':
                    formulaString = formulaString.replace('cost_price', newConnRenewal.cost_price);
                    break;
                  case 'dealer_price':
                    let dealerPackages = await DealerPackages.findOne({
                      where: { id: newConnection.packages.id, status_id: { '!=': Status.DELETED },
                    }});
  ...
  find: function (req, res) {
    ...
    const getConnRenewal = async () => {
      ...
      if (ConnRenewal_count < 1) {
        throw new CustomError('document not found', {
          status: 403
        });
      }
      let connRenewal = await ConnRenewal.find(queryObject).populate('connection');;
