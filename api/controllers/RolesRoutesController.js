...
module.exports = {
  ...
  create: function (req, res) {
    ...
    const process = async () => {
      const countRolesRoutes = await RolesRoutes.count({
        where: { roles: req.param('role_id'), routes: req.param('routes_id'), }
      });
      if (countRolesRoutes >= 1) {
  ...
  find: function (req, res) {
    ...
    const getRolesRoutes = async () => {
      ...
      if (RolesRoutes_count < 1) {
        throw new CustomError('RolesRoutes not found', {
          status: 403
        });
      }
  ...
  update: function (req, res) {
    ...
    const updateRolesRoutes = async () => {
      ...
      if (oldRolesRoutes < 1) {
        throw new CustomError('Invalid RolesRoutes  Id', {
          status: 403
        });
      }
